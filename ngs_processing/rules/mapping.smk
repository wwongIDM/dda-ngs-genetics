...
rule clean_sam:
    ...
    output:
        temp("mapped/{sample}-{unit}_cleanSam.sorted.bam")
    log:
...
rule fix_mates:
    input:
        "mapped/{sample}-{unit}_cleanSam.sorted.bam"
    output:
        temp("mapped/{sample}-{unit}_fixMate.sorted.bam")
    log:
        "logs/picard/fixmate/{sample}-{unit}.log"

    shell: """
        picard FixMateInformation I={input} O={output}
    """

rule mark_duplicates:
    input:
        "mapped/{sample}-{unit}_fixMate.sorted.bam"
    output:
